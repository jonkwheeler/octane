...
import { getOverflowAncestors, shift } from '@octanejs/floating-ui';
...
export function useFloatingTooltip<T extends HTMLElement = any>({
  ...
  const { x, y, elements, refs, update, placement } = useMantineFloating({
    placement: position,
    middleware: [
      shift({
        crossAxis: true,
  ...
  const handleMouseMove = useCallback(
    ({ clientX, clientY }: MouseEvent | OctaneMouseEvent<T, MouseEvent>) => {

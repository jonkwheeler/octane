import { use, useCallback, useEffect, useRef, useState } from 'octane';
import {
  useDelayGroup,
  useDismiss,
  useHover,
...
import { useMantineFloating } from '../../utils/Floating/use-mantine-floating';
...
export function useHoverCard(settings: UseHoverCard) {
  ...
  const { context, refs } = useMantineFloating({
    open: opened,

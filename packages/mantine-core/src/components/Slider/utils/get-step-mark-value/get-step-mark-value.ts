...
export function getNextMarkValue(
  currentValue: number,
  marks: { value: number; label?: OctaneNode }[]
) {
...
export function getFirstMarkValue(marks: { value: number; label?: OctaneNode }[]) {
  const sortedMarks = [...marks].sort((a, b) => a.value - b.value);
  return sortedMarks.length > 0 ? sortedMarks[0].value : 0;
}

export function getLastMarkValue(marks: { value: number; label?: OctaneNode }[]) {
  const sortedMarks = [...marks].sort((a, b) => a.value - b.value);

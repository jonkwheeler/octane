...
import type { TreeDragState, TreeNodeData } from './Tree.tsrx';
...
export interface TreeDragHandleProps {
  onMouseDown: (event: OctaneMouseEvent) => void;
}
...
export function useTreeNodeDragDrop({
  ...
  const handleDragStart = (event: OctaneDragEvent) => {
    if (withDragHandle && !isDragHandleActive) {
      return;
    }
  ...
  const handleDragOver = (event: OctaneDragEvent) => {
  ...
  const handleDragLeave = (event: OctaneDragEvent) => {
    const target = event.currentTarget as HTMLElement;
  ...
  const handleDrop = (event: OctaneDragEvent) => {
    event.preventDefault();
    event.stopPropagation();
  ...
  const handleDragEnd = (event: OctaneDragEvent) => {
    const target = event.currentTarget as HTMLElement;

...
interface UseComboboxTargetPropsInput {
  ...
  onKeyDown: ((event: OctaneKeyboardEvent<HTMLInputElement>) => void) | undefined;
  onClick: ((event: OctaneMouseEvent<HTMLInputElement>) => void) | undefined;
...
export function useComboboxTargetProps({
  ...
  const handleKeyDown = (event: OctaneKeyboardEvent<HTMLInputElement>) => {
    onKeyDown?.(event);
  ...
  const handleClick = (event: OctaneMouseEvent<HTMLInputElement>) => {

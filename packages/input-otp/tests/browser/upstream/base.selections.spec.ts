...
it('should replace selected char if another is pressed', async () => {
	...
	await input.pressSequentially('123'); await input.press('ArrowLeft'); await input.pressSequentially('1');
	expect(await input.inputValue()).toBe('121');
});

it('should replace multi-selected chars if another is pressed', async () => {
	const input = page.getByRole('textbox');
	await input.pressSequentially('123456'); await input.press('Shift+ArrowLeft'); await input.press('Shift+ArrowLeft'); await input.pressSequentially('1');

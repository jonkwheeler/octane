...
describe('pristine React 19 characterization (upstream tag source)', () => {
	it('render and mount succeed without nodeRef and install no native touchstart', () => {
		...
		expect(() => flushSync(() => root.render(React.createElement(DraggableCore, null, React.createElement('div'))))).not.toThrow();
		const child = container.firstElementChild;
		expect(add.mock.calls.some(([name], index) => name === 'touchstart' && add.mock.instances[index] === child)).toBe(false);
		root.unmount();
		add.mockRestore();
	});
	...
	it('retains child type/non-drag props/style/class/ref while replacing three handlers', () => {
		...
		const child = React.createElement('div', { id: 'kept', className: 'kept', style: { color: 'red' }, ref, ...originals });
		const core = instance({ children: child });
		const clone = core.render();
		expect(clone.type).toBe(child.type);
		expect(clone.props).toMatchObject({ id: 'kept', className: 'kept', style: { color: 'red' }, ref });
	...
	it('re-resolves replacement node/document for teardown, retaining original resources', () => {
		...
		expect(originalRemove).not.toHaveBeenCalledWith('touchstart', core.onTouchStart, expect.anything());
		expect(replacementRemove).toHaveBeenCalledWith('touchstart', core.onTouchStart, { capture: true, passive: false });
	});
});

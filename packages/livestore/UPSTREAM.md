...
byte and the recorded source/test inventory.
...
| `StoreRegistryContext` | Ported to Octane context at `src/StoreRegistryContext.tsrx` | `tests/lifecycle.test.ts` |
...
| `useStoreRegistry` | Ported to Octane context with the upstream override behavior | `tests/lifecycle.test.ts` |
...
| `useSyncStatus` | Ported to Octane's external-store subscription boundary | `tests/document-sync.test.ts`, `tests/ssr/server.test.ts` |
| `LiveList`, `LiveListProps` | Ported to keyed Octane `@for`; exported from root and `./experimental` | `tests/live-list.test.ts`, `typetests/public-api.test-d.ts` |
...
| `src/useStore.test.tsx` | Adapted by `tests/lifecycle.test.ts`; shared registry promise identity belongs to unchanged `StoreRegistry`, while Suspense, stable re-render, error propagation, store switching, and retention are binding contracts. React `useActionState` transition scheduling is not applicable to Octane. The upstream skipped `unusedCacheTime=0` case remains an upstream core issue, not a parity claim. |
| `src/useQuery.test.tsx` | Adapted by `tests/query.test.ts` and `tests/live-list.test.ts`; synchronous reads, commits, query identity changes, signals/query builders, per-store query derivation, list identity, and cleanup are the observable contracts. React-window-specific lifecycle stress is React-only; keyed `LiveList` supplies Octane's equivalent list integration evidence. Internal reactivity-graph snapshots are deliberately not copied because repository testing guidance forbids pinning private graph shape. |
| `src/useClientDocument.test.tsx` | Adapted by `tests/document-sync.test.ts`; ID changes, value/functional/external updates, query chaining, overwrite semantics, hydration, and cleanup are binding contracts. OpenTelemetry span snapshots exercise unchanged LiveStore instrumentation internals and are not re-snapshotted by the Octane binding. |
...
- Octane has no `useActionState`; the binding proves that a resolved store does
  not re-suspend across ordinary Octane updates instead.

...
describe('useContextBridge', () => {
	it('matches React by restoring every captured context over nested providers', async () => {
		...
		function Reader() { reactValue = [React.useContext(First), React.useContext(Second)]; return null; }
		function Capture() {
			const Bridge = reactUseContextBridge(First, Second);
			return React.createElement(First.Provider, { value: 'wrong-first' }, React.createElement(Second.Provider, { value: 'wrong-second' }, React.createElement(Bridge, null, React.createElement(Reader))));
		}
		...
		await act(async () => reactRoot.render(React.createElement(First.Provider, { value: 'alpha' }, React.createElement(Second.Provider, { value: 'beta' }, React.createElement(Capture)))));
		...
		octaneRoot.render(ContextBridgeScene, { first: 'alpha', second: 'beta', onValue: (value: string[]) => (octaneValue = value) });

...
import { FaceControls as ReactFaceControls, type FaceControlsApi as ReactFaceControlsApi, useFaceControls as useReactFaceControls } from '@react-three/drei/web/FaceControls.js';
import { createRoot as createOctaneRoot } from '@octanejs/three';
import { beforeAll, describe, expect, it, vi } from 'vitest';
import * as THREE from 'three';
import { FaceControls, type FaceControlsApi, type FaceControlsProps } from '../src/web/FaceControls.three.tsrx';
...
vi.mock('@react-three/drei/web/FaceLandmarker.js', () => ({ useFaceLandmarker: () => mocks.landmarker }));
vi.mock('../src/web/FaceLandmarker.three.tsrx', () => ({ useFaceLandmarker: () => mocks.landmarker }));
...
function renderer(canvas: HTMLCanvasElement): THREE.WebGLRenderer {
	return { domElement: canvas, outputColorSpace: THREE.SRGBColorSpace, toneMapping: THREE.NoToneMapping,
		render() {}, setPixelRatio() {}, setSize() {}, renderLists: { dispose() {} }, forceContextLoss() {}, dispose() {} } as unknown as THREE.WebGLRenderer;
}
...
async function mountPair(props: FaceControlsProps) {
	...
	function ContextCapture() { reactContext = useReactFaceControls(); return null; }
	...
	await act(async () => reactRoot.render(React.createElement(React.Fragment, null,
		React.createElement(ReactFaceControls, {
			...props as any,
			facemesh: { ...props.facemesh, children: React.createElement(React.Fragment, null, React.createElement('meshNormalMaterial'), React.createElement(ContextCapture)) },
			ref: (value: ReactFaceControlsApi | null) => { reactApi = value; },
		}),
	...
	octaneRoot.render(FaceControlsScene, { ...props, controlsRef: (value: FaceControlsApi | null) => { octaneApi = value; }, onContext: (value: FaceControlsApi) => { octaneContext = value; } });
	...
	return { reactRoot, octaneRoot, reactApi: reactApi as ReactFaceControlsApi, octaneApi: octaneApi as FaceControlsApi,
		reactContext: reactContext as ReactFaceControlsApi, octaneContext: octaneContext as FaceControlsApi, reactState };
}
...
describe('FaceControls', () => {
	it('matches manual result propagation, explicit defaults, Facemesh output, API and instant update', async () => {
		...
		const pair = await mountPair({ manualDetect: true, manualUpdate: true, faceLandmarkerResult: result,
			smoothTime: 0, eyes: true, eyesAsOrigin: false, depth: 0.3, offset: true, offsetScalar: 40,
			debug: true, facemesh: { name: 'overridden-by-controls', visible: false } });
		...
		const reactTarget = new THREE.Object3D(); reactTarget.position.set(1, 2, 3); reactTarget.rotation.set(0.1, 0.2, 0.3);
		const octaneTarget = reactTarget.clone();
		pair.reactApi.update(0.25, reactTarget);
		pair.octaneApi.update(0.25, octaneTarget);
		expect(pair.octaneRoot.store.getState().camera.position.toArray()).toEqual(pair.reactState.camera.position.toArray());
		expect(rounded(pair.octaneRoot.store.getState().camera.rotation.toArray().slice(0, 3) as number[])).toEqual(
			rounded(pair.reactState.camera.rotation.toArray().slice(0, 3) as number[]),
		);
		pair.octaneRoot.store.getState().camera.position.x += 1;
		expect(pair.octaneRoot.store.getState().camera.position.toArray()).not.toEqual(pair.reactState.camera.position.toArray());
		await unmountPair(pair);
	});

	it('matches automatic video-frame detection and result propagation', async () => {
		mocks.landmarker.detectForVideo.mockReset().mockReturnValue(FacemeshDatas.SAMPLE_FACELANDMARKER_RESULT);
		const callbacks: VideoFrameRequestCallback[] = [];
		const originalCreateElement = document.createElement.bind(document);
		const createElement = vi.spyOn(document, 'createElement').mockImplementation(((name: string, options?: ElementCreationOptions) => {
			...
			if (name === 'video') {
				...
				Object.defineProperty(video, 'requestVideoFrameCallback', { value: (callback: VideoFrameRequestCallback) => { callbacks.push(callback); return callbacks.length; }, configurable: true });
		...
		const pair = await mountPair({ manualDetect: false, manualUpdate: true, videoTexture: { src: '/face.mp4', start: false } });
		...
		expect(faceSnapshot(pair.octaneApi)).toEqual(faceSnapshot(pair.reactApi));
		expect(faceSnapshot(pair.octaneApi).positionCount).toBe(FacemeshDatas.SAMPLE_FACELANDMARKER_RESULT.faceLandmarks[0].length);
		await unmountPair(pair);
		createElement.mockRestore();
	});

	it('matches computeTarget, automatic smoothing, makeDefault and cleanup', async () => {
		const pair = await mountPair({ manualDetect: true, faceLandmarkerResult: FacemeshDatas.SAMPLE_FACELANDMARKER_RESULT as any,
			manualUpdate: false, makeDefault: true, smoothTime: 0.25, eyes: false, debug: false });

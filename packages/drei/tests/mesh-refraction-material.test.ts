...
describe('MeshRefractionMaterial', () => {
	it('matches React defines, uniforms, BVH initialization, and frame camera matrices', async () => {
		...
		await reactRoot.configure({ gl: renderer(canvas), frameloop: 'never', size: { width: 640, height: 360, top: 0, left: 0 }, dpr: 1 });
		await act(async () => reactRoot.render(React.createElement(React.Fragment, null,
			React.createElement('mesh', { ref: (value: THREE.Mesh) => (reactMesh = value) }, React.createElement('icosahedronGeometry', { args: [1, 1] }), React.createElement(ReactMeshRefractionMaterial, { envMap, aberrationStrength: 0.2, fastChroma: false, ior: 1.7, bounces: 4, color: 'cyan' })),
			React.createElement(Capture),
		)));
		let octaneMesh!: THREE.Mesh;
		const octane = await createOctaneThree(MeshRefractionMaterialScene, { envMap, aberrationStrength: 0.2, fastChroma: false, ior: 1.7, bounces: 4, color: 'cyan', meshRef: (value: THREE.Mesh) => (octaneMesh = value) }, { width: 640, height: 360 });

...
function renderer(canvas: HTMLCanvasElement): THREE.WebGLRenderer {
	return {
		...
		render() {}, setPixelRatio() {}, setSize() {}, renderLists: { dispose() {} }, forceContextLoss() {}, dispose() {},
...
function snapshot(mesh: THREE.Mesh) {
	const geometry = mesh.geometry as THREE.BufferGeometry & { parameters?: { text?: string; options?: Record<string, unknown> } };
	return {
		name: mesh.name,
...
describe('Text3D', () => {
	it('matches the pinned React component for text extraction, geometry options, material children, smoothing, and refs', async () => {
		...
		await reactThreeAct(async () => reactRoot.render(React.createElement(
			React.Suspense,
			{ fallback: null },
			React.createElement(ReactText3D, {
				ref: (value: THREE.Mesh) => (reactMesh = value), font, size: 1.25, height: 0.3,
				bevelEnabled: true, bevelSize: 0.025, letterSpacing: 0.1, lineHeight: 1.2, smooth: 0.001, name: 'text',
			}, 'Octane', 7, React.createElement('meshBasicMaterial', { color: 'hotpink' })),
		)));
		let octaneMesh!: THREE.Mesh;
		const octaneRoot = await createOctaneThree(Text3DScene, {
			meshRef: (value: THREE.Mesh) => (octaneMesh = value), font, size: 1.25, height: 0.3,
			bevelEnabled: true, bevelSize: 0.025, letterSpacing: 0.1, lineHeight: 1.2, smooth: 0.001,
		});
		await flush();

...
const packageRoot = resolve(repositoryRoot, 'packages/react-pdf');
...
describe('@octanejs/react-pdf packed consumer', () => {
	// @parity-case packed:react-pdf-consumer
	it('builds client/server imports and executes SSR without React', async () => {
		const root = await mkdtemp(join(tmpdir(), 'octane-react-pdf-packed-'));
		temporaryRoots.push(root);
		const packageDirectory = join(root, 'node_modules/@octanejs/react-pdf');
...
`,
		...
		await writeFile(
			...
			`import { Document } from '@octanejs/react-pdf';

import { useCallback, useMemo, useRef, useState, type OctaneNode } from 'octane';
import { getInputOnChange } from './get-input-on-change';
import { FormMode, GetInputPropsType } from './types';
import { shouldValidateOnChange } from './validate';

type UseFieldErrorResolver = (error: unknown) => OctaneNode;
...
> {
	...
	validate?: (value: T) => OctaneNode | Promise<OctaneNode>;
...
> {
	...
	validate: () => Promise<OctaneNode | void>;
	...
	setError: (error: OctaneNode) => void;
...
>({
	...
	const [error, setError] = useState<OctaneNode>(initialError || null);
	...
	const errorResolver: UseFieldErrorResolver = useMemo(
		() => resolveValidationError || ((err) => err as OctaneNode),
		[resolveValidationError],
	);
	...
	const getInputProps = ({ withError = true, withFocus = true, ...otherOptions }: any = {}) => {
		...
		const payload: any = type === 'input' ? { onInput: onChange } : { onChange };

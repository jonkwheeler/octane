...
export type FormErrors = Record<string, OctaneNode>;
...
export type FormValidateInput<Values> =
	FormRulesRecord<Values> | ((values: Values) => FormErrors | Promise<FormErrors>);
...
export type OnSubmit<Values, TransformedValues = Values> = (
	handleSubmit: (values: TransformedValues, event: SubmitEvent | undefined) => void | Promise<any>,
...
export interface UseFormInput<Values, TransformedValues = Values> {
	...
	resolveValidationError?: (error: unknown) => OctaneNode;
}

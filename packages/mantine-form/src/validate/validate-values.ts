...
function validateRulesRecord<T>(
	...
	const asyncTasks: Array<{ rulePath: string; promise: Promise<OctaneNode> }> = [];
...
const defaultResolveError = (err: unknown): OctaneNode =>
	err instanceof Error ? err.message : String(err);

export function validateValues<T>(
	validate: FormValidateInput<T> | undefined,
	values: T,
	resolveValidationError: (error: unknown) => OctaneNode = defaultResolveError,

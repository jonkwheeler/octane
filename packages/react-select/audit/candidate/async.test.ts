...
import {
	...
	type AsyncOption,
	type AsyncResult,
} from './browser/async-fixture.tsrx';

function ReactFixture(props: Record<string, unknown>) {
	const result = useReactAsync<AsyncOption, false, never, Record<string, unknown>>(
		props as never,
	);
...
describe('useAsync SSR parity', () => {
	it.each([
		...
		{ defaultOptions: true },
	])('matches React initial server state for %#', (asyncProps) => {

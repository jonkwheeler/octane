...
function canonicalMarkup(html: string, removeLiveRegion = false) {
	...
	if (removeLiveRegion) {
		for (const element of window.document.body.querySelectorAll('[role="log"], [id$="-live-region"]')) {
			element.remove();
		}
	}
...
describe('composed Select entry-point SSR parity', () => {
	it.each([
		...
		const react = renderToStaticMarkup(
			React.createElement(ReactComponent<SelectOption> as React.ComponentType<Record<string, unknown>>, props),
		);

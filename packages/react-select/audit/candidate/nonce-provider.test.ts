...
describe('NonceProvider parity', () => {
	...
	it.each(['nonce-a', 'nonce-b'])('applies cache key %s and the CSP nonce to every SSR style', (cacheKey) => {
		...
		const octaneStyles = [...octane.css.matchAll(/<style data-octane="([^"]+)" nonce="([^"]+)">/g)];

...
describe('findMatchingHitRegions', () => {
	...
	test('group with no separator', () => {
		...
		expect(serialize(mockPointerEvent({ clientX: 50 }), getMountedGroups())).toMatchInlineSnapshot(`
        "[
          {
            "panels": [
              "group-1-left",
	...
	test('group with separator', () => {
		...
		expect(serialize(mockPointerEvent({ clientX: 60 }), getMountedGroups())).toMatchInlineSnapshot(`
        "[
          {
            "panels": [
              "group-1-left",
	...
	test('nested groups', () => {
		...
		expect(serialize(mockPointerEvent({ clientX: 50, clientY: 25 }), getMountedGroups()))

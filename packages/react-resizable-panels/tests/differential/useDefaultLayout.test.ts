import { act as octaneAct, cleanup as cleanupOctane, renderHook as renderOctaneHook } from "@octanejs/testing-library";
import { useDefaultLayout as useOctaneDefaultLayout, type LayoutStorage } from "@octanejs/react-resizable-panels";
...
describe("react-resizable-panels useDefaultLayout differential", () => {
  ...
  test("identical deprecated debounce checkpoints and cancellation", () => {
    ...
    const reactRoot = createRoot(reactContainer);
    function ReactHarness() {
      reactCurrent = useReactDefaultLayout({ id: "workspace", storage: reactStorage.storage, debounceSaveMs: 20 });
      return null;
    }
    reactAct(() => reactRoot.render(createElement(ReactHarness)));
    const octane = renderOctaneHook(() =>
      useOctaneDefaultLayout({ id: "workspace", storage: octaneStorage.storage, debounceSaveMs: 20 })
    );

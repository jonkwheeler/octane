...
import { runPristineUpstreamSuite } from '../../../scripts/react-parity/vaul-pristine-runtime.mjs';

const repoRoot = resolve(dirname(fileURLToPath(import.meta.url)), '../../..');

type Identity = { file: string; fullName: string; status?: string };

// @parity-case pristine:vaul-original-suite
it(
	'runs the pinned vaul 1.1.2 Playwright suite unchanged',
	function runsPinnedVaulSuite() {
		const expected = JSON.parse(
			readFileSync(resolve(repoRoot, 'packages/vaul/audit/pristine-runtime.json'), 'utf8'),
		).tests as Identity[];

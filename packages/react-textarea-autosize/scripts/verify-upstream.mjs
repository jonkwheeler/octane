...
const sourceFiles = (await readdir(new URL('upstream/src/', root), { recursive: true, withFileTypes: true })).filter((entry) => entry.isFile());
...
if (JSON.stringify(actualPaths) !== JSON.stringify(expectedPaths)) {
	throw new Error(`upstream source inventory mismatch\nexpected: ${expectedPaths.join(', ')}\nactual: ${actualPaths.join(', ')}`);
}
for (const [file, digest] of Object.entries(manifest.files)) {
	const actual = createHash('sha256').update(await readFile(new URL(`upstream/src/${file}`, root))).digest('hex');
...
if (pkg.name !== 'react-textarea-autosize' || pkg.version !== '8.5.9' || pkg.license !== 'MIT') throw new Error('upstream package identity mismatch');
const bindingPkg = JSON.parse(await readFile(new URL('package.json', root), 'utf8'));
const leafPaths = (value, prefix = '') =>
	Object.entries(value).flatMap(([key, child]) =>
		typeof child === 'object' && child !== null ? leafPaths(child, `${prefix}${key}.`) : `${prefix}${key}`,
	);
...
if (JSON.stringify(bindingConditions) !== JSON.stringify(upstreamConditions)) {
	throw new Error(`package export condition mismatch\nexpected: ${upstreamConditions.join(', ')}\nactual: ${bindingConditions.join(', ')}`);
}
for (const unpublished of ['audit', 'scripts', 'tests', 'typetests', 'upstream']) {
	if (bindingPkg.files.includes(unpublished)) throw new Error(`${unpublished}/ must remain unpublished evidence`);
}
...
console.log(`verified react-textarea-autosize@8.5.9: ${sourceFiles.length} byte-pinned source/test artifacts`);

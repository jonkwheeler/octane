...
beforeAll(async () => {
	...
	viteServer = await createServer({
		...
		resolve: {
			alias: [
				{ find: /^@octanejs\/react-popper$/, replacement: bindingSource },
...
describe('@octanejs/react-popper real-browser parity', () => {
	// @parity-case browser:react-popper-chromium
	it(
		'chromium: positions, updates, wires arrows/virtual refs, and cleans up',
		() => runBrowserCase('chromium'),
		60_000,
	);
	// @parity-case browser:react-popper-firefox
	it(

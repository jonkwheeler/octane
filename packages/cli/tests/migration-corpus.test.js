...
describe('migration compatibility corpus', () => {
	...
	it('keeps unported Mantine extensions blocked without hiding supported dependencies', () => {
		...
		expect(bySpecifier.get('@mantine/notifications')).toMatchObject({
			classification: 'supported',
			replacement: '@octanejs/mantine-notifications',
			evidence: 'binding-catalog:react-package',
		});
		...
		expect(bySpecifier.get('@mantine/code-highlight')).toMatchObject({
			classification: 'supported',
			replacement: '@octanejs/mantine-code-highlight',
			evidence: 'binding-catalog:react-package',
		});
		...
		expect(bySpecifier.get('@mantine/dropzone')).toMatchObject({
			classification: 'supported',

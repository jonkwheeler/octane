...
export type NumberFormatChangeEvent<T = Element> = Event & {
  currentTarget: T & EventTarget;
  target: T & EventTarget;
};
export type NumberFormatFocusEvent<T = Element> = FocusEvent & {
  currentTarget: T & EventTarget;
  target: T & EventTarget;
};
export type NumberFormatKeyboardEvent<T = Element> = KeyboardEvent & {
  currentTarget: T & EventTarget;
  target: T & EventTarget;
};
export type NumberFormatMouseEvent<T = Element> = MouseEvent & {
  currentTarget: T & EventTarget;
  target: T & EventTarget;
};
...
export type InputAttributes = Omit<
  Octane.JSX.IntrinsicElements['input'],
...
type NumberFormatProps<Props, BaseType = InputAttributes> = Props &
  ...
  Omit<BaseType, keyof Props | 'ref'> & {
    customInput?: (props: BaseType) => OctaneNode;
  };
...
type NumberFormatBase = {
  ...
  renderText?: (formattedValue: string, otherProps: Partial<NumberFormatBase>) => OctaneNode;
  format?: FormatInputValueFunction;
  removeFormatting?: RemoveFormattingFunction;
  getInputRef?: ((el: HTMLInputElement) => void) | Octane.Ref<any>;

...
| `universal-template-events` | universal-template-events | none (Node-only) | shape-stable handler updates across 256 and 2,048 retained native event sites through the fallback collapsed-template host capability, with host identity, latest-handler dispatch, and redundant-command controls |

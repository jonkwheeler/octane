...
test('verifies an arbitrary binding classification ledger', async (t) => {
	...
	t.after(() => rm(root, { recursive: true, force: true }));
	await cp(
		new URL('../../packages/streamdown/tests', import.meta.url),
		join(root, 'packages/streamdown/tests'),
		{ recursive: true },
	);
	for (const file of ['test-classifications.json', 'react-parity.json']) {
		await cp(
			new URL(`../../packages/streamdown/audit/${file}`, import.meta.url),
			join(root, `packages/streamdown/audit/${file}`),
			{ recursive: true },
		);
	}
	...
	assert.throws(
		...
		/every port-authored streamdown test must have exactly one classification/,
	);
});

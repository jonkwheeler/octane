...
import { buildTypeInventory } from './waypoint-types-lib.mjs';

async function fixture() {
	const root = await mkdtemp(join(tmpdir(), 'waypoint-types-'));
	const upstreamRoot = join(root, 'upstream');
	const adaptedRoot = join(root, 'adapted');
	await cp(
		new URL('../../packages/waypoint/typetests/pristine', import.meta.url),
		upstreamRoot,
		{ recursive: true },
	);
	await cp(
		new URL('../../packages/waypoint/typetests/adapted', import.meta.url),
		adaptedRoot,
		{ recursive: true },
	);
...
test('rejects retargeting an adapted public import', async function rejectsRetargetedImport(t) {
	...
	await writeFile(
		file,
		source.replace("from '@octanejs/waypoint'", "from '@octanejs/waypoint/src/types.ts'"),
	);
	assert.throws(function run() {
		buildTypeInventory(value.root, value.config);

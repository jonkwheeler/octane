...
const CONFIG = 'packages/react-pdf/audit/test-classifications.json';
const MANIFEST = 'packages/react-pdf/audit/react-parity.json';
...
export function verifyReactPdfTestClassifications(root) {
	const testsRoot = resolve(root, 'packages/react-pdf/tests');
	...
	if (JSON.stringify(discovered) !== JSON.stringify(declared)) {
		throw new Error('every port-authored react-pdf test must have exactly one classification');
	}

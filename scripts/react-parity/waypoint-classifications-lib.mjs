...
const CONFIG = 'packages/waypoint/audit/test-classifications.json';
const MANIFEST = 'packages/waypoint/audit/react-parity.json';
...
export function verifyWaypointTestClassifications(root) {
	const testsRoot = resolve(root, 'packages/waypoint/tests');
	...
	if (JSON.stringify(discovered) !== JSON.stringify(declared)) {
		throw new Error('every port-authored waypoint test must have exactly one classification');
	}

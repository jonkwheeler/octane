...
describe('react-resizable-panels public type parity evidence', () => {
  test('strict Octane public probes compile with their negative assertions', () => {
    execFileSync(path.join(repo, 'node_modules/.bin/tsrx-tsc'), ['--noEmit', '-p', path.join(packageRoot, 'audit/type-probes/tsconfig.json')], { cwd: repo });
  });

  test('the entry point explicitly exports exactly the pinned public types', () => {
    const expected = JSON.parse(readFileSync(path.join(packageRoot, 'audit/public-api.json'), 'utf8')).types.sort();

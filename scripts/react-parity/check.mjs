...
import { verifyReactPopperTypes } from './react-popper-types-lib.mjs';
...
try {
	verifyReactPopperTypes(REPO);
} catch (error) {
	errors.push(`react-popper type evidence is invalid: ${error.message}`);
}
try {
	verifyReactPopperTestClassifications(REPO);
} catch (error) {
	errors.push(`react-popper test classifications are invalid: ${error.message}`);
}

...
export function verifyPortTestClassifications(root, binding = 'hook-form') {
	const configFile = `packages/${binding}/audit/test-classifications.json`;
	const manifestFile = `packages/${binding}/audit/react-parity.json`;
	...
	const discovered = readdirSync(testsRoot, { recursive: true, withFileTypes: true })
		...
		.filter((path) => binding !== 'hook-form' || !path.includes('/tests/upstream/'))
		.sort();
	const configPath = resolve(root, configFile);
	if (!existsSync(configPath)) throw new Error(`missing port-test classifications: ${configFile}`);

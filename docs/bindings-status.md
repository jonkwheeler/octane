...
| [`@octanejs/waypoint`](#octanejswaypoint) | `react-waypoint@10.3.0` | Complete Waypoint component and position constants with vertical/horizontal geometry, offsets, ancestor selection, and rapid-crossing callbacks. | Public node and prop types use Octane structural types and do not import React; Octane's ref-as-prop model removes the upstream forwardRef detection branch; A custom child must be supplied through the children prop so it remains an inspectable element descriptor; nested TSRX children compile to an opaque render block | Supported. The marker renders on the server and measurement/listeners begin after client mount. | 2026-07-30 |
...
Complete Waypoint component and position constants with vertical/horizontal geometry, offsets, ancestor selection, and rapid-crossing callbacks.

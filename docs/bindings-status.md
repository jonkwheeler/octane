...
The central status table for the 57 `@octanejs/*` framework bindings.
...
| [`@octanejs/waypoint`](#octanejswaypoint) | `react-waypoint@10.3.0` | Waypoint component and position constants at react-waypoint 10.3.0, with vertical/horizontal geometry, offsets, ancestor selection, and rapid-crossing callbacks. Node-suite pristine/adapted runtime and repo-authored type lanes are registered with react-parity:check; the Karma browser suite remains deferred. | Public node and prop types use Octane structural types and do not import React; Octane's ref-as-prop model removes the upstream forwardRef detection branch; A custom child must be supplied through the children prop so it remains an inspectable element descriptor; nested TSRX children compile to an opaque render block; Upstream browser Karma suite is not yet adapted; scroll-geometry parity remains incomplete until that lane lands. See UPSTREAM.md | Supported. The marker renders on the server and measurement/listeners begin after client mount. | 2026-07-30 |
...
[`packages/animejs`](../packages/animejs) `0.0.1` — ports `animejs@4.5.0`. Status data: [`packages/animejs/status.json`](../packages/animejs/status.json).
...
Waypoint component and position constants at react-waypoint 10.3.0, with vertical/horizontal geometry, offsets, ancestor selection, and rapid-crossing callbacks. Node-suite pristine/adapted runtime and repo-authored type lanes are registered with react-parity:check; the Karma browser suite remains deferred.

Known divergences:

- Public node and prop types use Octane structural types and do not import React.
- Octane's ref-as-prop model removes the upstream forwardRef detection branch.
...
- Upstream browser Karma suite is not yet adapted; scroll-geometry parity remains incomplete until that lane lands. See UPSTREAM.md.

...
- Bounded differential parity: the required manifest lane authenticates five exact cases against curated React references derived from the pinned upstream shapes. The references include port-selected class hooks and icon resolution, so all five prove runtime equivalence between Octane and React, not byte-identical upstream fidelity. Portal'd content is excluded from the byte compare — overlay/menu content parity is covered by Octane-only behavioral contracts.

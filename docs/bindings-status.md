...
[`packages/react-window`](../packages/react-window) `0.0.1` — ports `react-window@2.3.0`. Status data: [`packages/react-window/status.json`](../packages/react-window/status.json).

Complete exact port of the react-window 2.3.0 root surface: List, Grid, getScrollbarSize, useDynamicRowHeight, four imperative-ref hooks, and all eight public types. The byte-locked pristine React suite and generated Octane adaptation each execute all 14 upstream files and all 117 cases; shared differential, SSR, hydration, and public type lanes are required by the React parity manifest.

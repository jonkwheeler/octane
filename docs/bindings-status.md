...
| [`@octanejs/intersection-observer`](#octanejsintersection-observer) | `react-intersection-observer@10.1.0` | Public runtime surface at react-intersection-observer 10.1.0: useInView, useOnInView, InView, observe, defaultFallbackInView, and test utilities. Pinned upstream unit suite and one-for-one adapted runtime/type probes are registered with react-parity:check; browser-provider cases remain outstanding. | Public node and prop types use Octane structural types and do not import React; InView is implemented as a function component while preserving the upstream render-prop and wrapper behavior; In TSRX, an InView render function must be supplied through the children prop; nested children compile to an opaque render block; Browser-provider upstream cases (`browser.test.tsx`) are not yet in the jsdom parity lanes; see UPSTREAM.md | Supported. Observation begins in an effect; initialInView controls deterministic server output. | 2026-07-30 |
...
Public runtime surface at react-intersection-observer 10.1.0: useInView, useOnInView, InView, observe, defaultFallbackInView, and test utilities. Pinned upstream unit suite and one-for-one adapted runtime/type probes are registered with react-parity:check; browser-provider cases remain outstanding.
...
- Browser-provider upstream cases (`browser.test.tsx`) are not yet in the jsdom parity lanes; see UPSTREAM.md.

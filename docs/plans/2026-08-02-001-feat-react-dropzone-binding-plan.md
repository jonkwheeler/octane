...
- **Objective:** Ship `@octanejs/react-dropzone` as the exact mapped Octane binding for `react-dropzone@20.0.0`, preserving its root runtime, complete type surface, package conditions, file-selection state machine, and observable browser behavior.
...
- R1. Publish `@octanejs/react-dropzone` with exact mapped root runtime and types plus the pinned root `types`/`import`/`require` conditions and `./package.json` export. Point published conditions at authored Octane source in accordance with repository policy, while proving equivalent consumer resolution.
...
- Package-name aliasing from `react-dropzone` is outside scope; consumers map imports to `@octanejs/react-dropzone`.
...
- Input, drag/drop, paste, File System Access, refs, prop getters, validation, supersession, SSR, streaming, hydration, and trusted Chromium behavior match or carry a narrow documented Octane divergence with executable evidence.
...
- KTD1. **Mirror the two-module upstream source boundary.** Vendor byte-exact `src/index.tsx` and `src/utils/index.ts`; mirror them under `packages/react-dropzone/src/`; reuse `attr-accept` and `file-selector`; adapt only React imports, hook slots, component rendering, refs, and event types. Governs R4-R13.
...
- **Requirements:** R1-R6, R8-R15.
- **Dependencies:** None.
- **Files:** `packages/react-dropzone/upstream/`, `packages/react-dropzone/UPSTREAM.md`, `packages/react-dropzone/package.json`, `packages/react-dropzone/tsconfig.json`, `packages/react-dropzone/audit/public-api.json`, `packages/react-dropzone/audit/npm-files.json`, `packages/react-dropzone/audit/upstream-files.json`, `packages/react-dropzone/audit/runtime-inventories/`, `packages/react-dropzone/audit/type-inventories/`, `packages/react-dropzone/audit/verify-provenance.mjs`, `packages/react-dropzone/tests/pristine/upstream-runtime.test.ts`, `packages/react-dropzone/tests/probes/dropzone.tsrx`, `packages/react-dropzone/tests/probes/architecture.test.ts`, `packages/react-dropzone/tests/probes/server.test.ts`, `packages/react-dropzone/tests/probes/hydration.test.ts`, `packages/react-dropzone/tests/probes/browser/`, `packages/react-dropzone/tests/probes/packed-exports.test.mjs`, `packages/react-dropzone/tests/probes/consumers/`, `packages/react-dropzone/typetests/pristine/`, `pnpm-workspace.yaml`, `pnpm-lock.yaml`, `vitest.config.js`.
- **Approach:** Hash the npm tarball separately from the canonical commit archive; vendor byte-exact source, tests, snapshot, configs, license, and required support. Generate package-condition, runtime/type-export, source/test, static-registration, collection-time, execution, snapshot, and type-program inventories. Run the upstream Vitest suite unchanged with pinned React/Vitest/jsdom and run both upstream TypeScript commands/projects unchanged. Scaffold only enough authored package source to test public prop getters, root/input spreads, arbitrary props, internal/user refs and `refKey`, imperative `open`, hidden-input delivery, browser-created DataTransfer drop, controlled async extraction/validator supersession, global/document listener cleanup, browser-global-free render, stream/hydration adoption, and packed ESM/CJS/types/package-json resolution. Compile the imported hook using both candidate slot strategies only as needed; select the simplest proven strategy. Use `node scripts/scaffold-react-port.mjs` to produce the exhaustive case ledger, but do not commit unresolved todos.
...
- **Requirements:** R8-R12.
- **Dependencies:** U1.
- **Files:** `packages/react-dropzone/src/utils/index.ts`, `packages/react-dropzone/tests/adapted/utils/index.spec.ts`, `packages/react-dropzone/tests/differential/utils.test.ts`, `packages/react-dropzone/typetests/accept.tsx`, `packages/react-dropzone/typetests/events.tsx`, `packages/react-dropzone/typetests/validator.tsx`.
...
- **Dependencies:** U1-U2.
- **Files:** `packages/react-dropzone/src/index.tsrx`, `packages/react-dropzone/src/index.tsrx.d.ts`, `packages/react-dropzone/tests/adapted/index.spec.tsx`, `packages/react-dropzone/tests/differential/component.test.ts`, `packages/react-dropzone/tests/_fixtures/dropzone.tsrx`, `packages/react-dropzone/typetests/basic.tsx`, `packages/react-dropzone/typetests/hook.tsx`, `packages/react-dropzone/typetests/plugin.tsx`, `packages/react-dropzone/typetests/refs.tsx`.
...
- **Requirements:** R8-R13.
- **Dependencies:** U2-U3.
- **Files:** `packages/react-dropzone/src/index.tsrx`, `packages/react-dropzone/tests/adapted/index.spec.tsx`, `packages/react-dropzone/tests/differential/acquisition.test.ts`, `packages/react-dropzone/tests/differential/supersession.test.ts`, `packages/react-dropzone/tests/browser/index.html`, `packages/react-dropzone/tests/browser/main.ts`, `packages/react-dropzone/tests/browser/react-dropzone.browser.test.ts`, `packages/react-dropzone/tests/_fixtures/acquisition.tsrx`, `packages/react-dropzone/typetests/file-dialog.tsx`, `packages/react-dropzone/typetests/validator.tsx`.
...
- **Dependencies:** U1-U4.
- **Files:** `packages/react-dropzone/tests/pristine/`, `packages/react-dropzone/tests/adapted/`, `packages/react-dropzone/tests/differential/`, `packages/react-dropzone/tests/ssr/`, `packages/react-dropzone/tests/hydration/`, `packages/react-dropzone/tests/browser/`, `packages/react-dropzone/typetests/`, `packages/react-dropzone/audit/react-parity.json`, `packages/react-dropzone/audit/test-classifications.json`, `packages/react-dropzone/audit/transformation-ledger.json`, `packages/react-dropzone/audit/runtime-inventories/`, `packages/react-dropzone/audit/type-inventories/`, `scripts/react-parity/react-dropzone-*.mjs`, `scripts/react-parity/check.mjs`, `scripts/react-parity/harness-lib.test.mjs`, `scripts/react-parity/react-dropzone-*.test.mjs`, `vitest.config.js`.
- **Approach:** Register unchanged pristine runtime and type lanes, exhaustive adapted runtime and type lanes, differential lanes, SSR/stream/hydration lanes, and required Chromium lanes in `audit/react-parity.json`. Inventory every upstream artifact, static registration, actual collected/executed identity, snapshot, type file, and structural assertion/negative group. Record only permitted transformations such as import roots, `.tsx` fixture conversion to `.tsrx`, React event/renderable type mapping, and forwardRef-to-ref-prop authoring; reject all others. Classify every port-authored test exactly once. Generalize the existing hook-form verifier libraries where their logic is reusable, retain existing hook-form coverage, and add negative controls for every evidence failure mode.
- **Test scenarios:** Missing/extra/renamed/duplicated/skipped/todo/expected-failure/unexecuted runtime cases fail; stale or fake-title inventories fail; removed snapshots/assertions/type files/negative programs fail; undeclared transformations and fixture/provenance drift fail; a locally runnable lane omitted from global `react-parity:check` fails. SSR renders default/disabled/custom-state fixtures without browser access; streaming settles deterministic async children; hydration adopts exact root/input nodes, preserves permissible pre-hydration focus/value/file state, attaches refs/listeners once, remains interactive, and reports no mismatch. Production/server transform variants and Chromium page diagnostics are included.
...
- **Dependencies:** U1-U5.
- **Files:** `packages/react-dropzone/package.json`, `packages/react-dropzone/README.md`, `packages/react-dropzone/LICENSE`, `packages/react-dropzone/UPSTREAM.md`, `packages/react-dropzone/status.json`, `packages/react-dropzone/tests/adoption/`, `packages/react-dropzone/tests/packed/`, `package.json`, `pnpm-workspace.yaml`, `pnpm-lock.yaml`, `vitest.config.js`, `scripts/check-package-packs.mjs`, `playground/octane/`, `website/src/content/bindings.json`, `.changeset/`, `docs/bindings-status.md`, `docs/binding-parity-gaps.md`, generated package/CLI inventories.
...
| PR boundary | U6 | Exactly one isolated binding PR contains the complete package and evidence; any Octane prerequisite or future pin remains separate. |

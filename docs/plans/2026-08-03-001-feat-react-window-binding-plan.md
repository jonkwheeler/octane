...
- **Objective:** Add `@octanejs/react-window` as an exact, source-accounted port of `react-window@2.3.0`, with executable React parity, SSR/hydration, browser virtualization, type, provenance, and packed-consumer evidence.
...
Applications importing the current `react-window` package should be able to map that dependency to `@octanejs/react-window` without accepting a merely similar virtualization API. The binding targets the current v2 package contract, not the incompatible v1 API that older documentation and ecosystem memory often imply.
...
- KTD3. **Port module by module from vendored source.** Mirror upstream `lib/` ownership in `packages/react-window/src/`; preserve framework-neutral math/utilities with source correspondence and adapt React-owned hooks/components to Octane instead of wrapping TanStack Virtual. Governs R2, R3, R5, and R9.
...
flowchart TB
  ...
  C --> D
  D --> E["@octanejs/react-window public package"]
...
- **Dependencies:** None.
- **Files:** `packages/react-window/upstream/**`, `packages/react-window/UPSTREAM.md`, `packages/react-window/audit/**`, `packages/react-window/tests/audit/**`, `packages/react-window/package.json`.
- **Approach:**
...
- **Dependencies:** U1.
- **Files:** `packages/react-window/tests/feasibility/**`, `packages/react-window/src/internal.ts`, `vitest.config.js`.
...
- **Dependencies:** U1, U2.
- **Files:** `packages/react-window/src/core/**`, `packages/react-window/src/hooks/**`, `packages/react-window/src/utils/**`, `packages/react-window/src/types.ts`, `packages/react-window/tests/upstream/**`, `packages/react-window/tests/differential/**`.
...
- **Dependencies:** U3.
- **Files:** `packages/react-window/src/components/list/**`, `packages/react-window/src/components/grid/**`, `packages/react-window/src/index.ts`, `packages/react-window/tests/runtime/**`, `packages/react-window/tests/differential/**`, `packages/react-window/tests/ssr/**`, `packages/react-window/tests/types/**`.
...
- **Files:** `packages/react-window/tests/browser/**`, browser fixture routes under `packages/react-window/tests/browser/fixtures/**`, `vitest.config.js`, `packages/react-window/audit/react-parity.json`.
...
- **Files:** `packages/react-window/package.json`, `packages/react-window/README.md`, `packages/react-window/status.json`, `.changeset/*.md`, `pnpm-workspace.yaml`, `vitest.config.js`, catalog/status/CLI/MCP mapping sources and generated artifacts, playground/consumer fixtures, package-pack validation sources.
...
- **Test scenarios:**
  ...
  - `react-window` dependency mapping resolves to `@octanejs/react-window` through CLI and MCP catalogs with exact focused tests.
...
- `@octanejs/react-window` exposes every pinned v2.3.0 runtime and type export with no silent gap or undocumented divergence.
